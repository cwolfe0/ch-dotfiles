    ...
    $J('#Lnk_SendGifts').click(function () {
        ...
        $J('.selectedSell').each(function () {
            ...
            if (rgItem.appid == 753 && rgItem.contextid == 1) {
                var isSent = false;
                var description = rgItem.owner_descriptions;
                description.each(function (desc, i) {
                    isSent = !desc.value.startsWith('Sent to');
                });
                if (isSent) {
                    url += rgItem.id + '/';
                }
