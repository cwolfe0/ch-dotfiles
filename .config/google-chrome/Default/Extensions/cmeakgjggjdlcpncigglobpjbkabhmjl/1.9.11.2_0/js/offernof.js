...
var GetExchangeRate = function (callback) {
    if (!exchangerate || (new Date() - csgofastlastcache) > 14400000) {
        $.ajax({
            // url: 'http://steaminventoryhlp.org/api/test.json',
            url: 'http://query.yahooapis.com/v1/public/yql',
            data: {
              q: 'select id, Name, Rate from yahoo.finance.xchange where pair in ("USDGBP","USDEUR","USDCHF","USDRUB","USDBRL","USDJPY","USDNOK","USDIDR","USDMYR","USDPHP","USDSGD","USDTHB","USDVND","USDKPW","USDTRY","USDUAH","USDMXN","USDCAD","USDAUD","USDNZD","USDCNY","USDINR","USDCLP","USDPEN","USDCOP","USDZAR","USDHKD","USDTWD","USDSAR","USDAED")',
              format: 'json',
              env: 'store://datatables.org/alltableswithkeys'
            }
        }).done(function (response) {
            var respRates = response.query.results.rate || [];
            if (respRates.length) {
              var rates = {};
              respRates.forEach(function (rate) {
                var curName = rate.Name.split('/')[1];
                rates[curName] = parseFloat(rate.Rate);
              });
              exchangerate = {
                  data: {"base":"USD", rates:rates},
                  cachedate: new Date()
              };
              if (callback) {
    ...
    $.ajax({
        ...
        if (response && response.hasOwnProperty('trade_offers_received') && response.trade_offers_received.length) {
            ...
            active_offers.forEach(function (offer) {
                if (items.autodecline && offer.trade_offer_state === g_tradeOfferState.InvalidItems) {
                    $.ajax({
                        method: 'POST',
                ...
                } else if (!_oHistory[offer.tradeofferid]) {
                    haveOffers = true;
                    $.ajax({
                        method: 'GET',
            ...
            if (haveOffers && items.shownotify) {
                var soundA = null;
                if (soundFile == 'custom') {
