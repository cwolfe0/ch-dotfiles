        ...
        if (items.apikey === '' && (items.userUrl === null || items.userUrl === '//steamcommunity.com/my/')) {
            $.ajax({
                ...
                if ($(response).find('.mainLoginPanel').length) {
                    $('#Div_Offers').html('').append(chrome.i18n.getMessage('tradeoffer_signedout'));
...
var nofSteam = function (idOffer, img, name) {
    ...
    var refuseLink = $('<a class="refuse-bt" href="#" >Quick refuse</a>');
    ...
    if (quickrefuse) {
        ...
        refuseLink.click(function (e) {
            ...
            if (window.qrdelay) {
                ...
                $(this).html('Cancel (' + (window.qrdelay < 10 ? '0' : '') + window.qrdelay + ')');
...
function getTradesByParsing(userUrl) {
    $.ajax({
        ...
        if (num > 0) {
            ...
            $('#Div_Offers').prepend('<div style="margin-bottom:20px;">' + chrome.i18n.getMessage('tradeoffer_nooffers') + '</div>');
            chrome.browserAction.setBadgeText({text: ""});
        }
        ...
        if (response.status === 429 && response.responseText === '') {
            ...
            $('#Div_Offers').html('').append('Steam: ' + response.status + ' ' + response.statusText);
        }
    });
}
...
function getTradesByAPI() {
    ...
    $.ajax({
        ...
        if (response && response.hasOwnProperty('trade_offers_received') && response.trade_offers_received.length) {
            ...
            offers.forEach(function (offer) {
                $.ajax({
                    ...
                    if (name !== undefined && avatar !== undefined) {
                        var header = 'You have offer from ' + name.textContent + (offer.message.length ? ': ' + offer.message : '');
            ...
            offersDivBlock.prepend('<div style="margin-bottom:20px;">You have ' + offers.length + ' trade offer(s)<br /> <a href="#" id="lnk_openall">Open all</a> <a href="#" id="lnk_declineall">Decline all</a> </div>');
            $('#lnk_openall').click(function () {
                openAll();
                return false;
            });
            ...
            chrome.browserAction.setBadgeText({text: "" + offers.length});
        } else {
            offersDivBlock.prepend('<div style="margin-bottom:20px;">' + chrome.i18n.getMessage('tradeoffer_nooffers') + '</div>');
            chrome.browserAction.setBadgeText({text: ""});
        }
    }).fail(function (result) {
        offersDivBlock.append('Oops. Something went wrong (Steam Web API).');
    });
}
    ...
    $.ajax({
        ...
        success: function (response) {
            var receivePrice = 0, givePrice = 0,
                itemsToReceive = collectItems(response.response.offer.items_to_receive),
                itemsToGive = collectItems(response.response.offer.items_to_give);

            if (itemsToReceive.length) {
                $('.offer-' + offerId + ' div:first').append('<span class="offer-price">Their price: <span class="price-theirs-' + offerId + '"></span></span>');
            }
            if (itemsToGive.length) {
                $('.offer-' + offerId + ' div:first').append('<span class="offer-price">Your price: <span class="price-yours-' + offerId + '"></span></span>');
            }

            for (var i in response.response.descriptions) {
                if (response.response.descriptions[i].appid === undefined) return true;

                var item = response.response.descriptions[i];
...
function collectItems(elements) {
    ...
    var data = [];
    for (var i in items) {
        var id = [items[i].appid, items[i].classid, items[i].instanceid].join('_');
        data.push(id);
    }
    return data;
}
